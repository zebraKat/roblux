...
CMDs[#CMDs + 1] = {NAME = 'massplay', DESC = 'MassPlays takes all boombos and plays together'}
CMDs[#CMDs + 1] = {NAME = 'timeposition', DESC = 'timeposition is used for syncing and doing so many stuff idk'}
...
CMDs[#CMDs + 1] = {NAME = 'pushaura', DESC = 'push aura for ragdoll engine'}
...
CMDs[#CMDs + 1] = {NAME = 'autoflingpirate', DESC = 'flingsnoway!!'}
CMDs[#CMDs + 1] = {NAME = 'autoflingR6', DESC = 'flingsnoway!!'}
...
CMDs[#CMDs + 1] = {NAME = 'muteall', DESC = 'Mutes all boomboxes'}
CMDs[#CMDs + 1] = {NAME = 'unmuteall', DESC = 'Unmutes all boomboxes'}
...
addcmd('muteall',{},function(args, speaker)
StopTheMusic = false
spawn(function()
while wait() do
...
addcmd('unmuteall',{},function(args, speaker)
StopTheMusic = true
end)

addcmd('pushaura',{},function(args, speaker)
...
for i,v in pairs(workspace:GetChildren()) do
    if v:IsA("Model") and v.Name ~= game.Players.LocalPlayer.Name then
        pcall(function()
                if v.HumanoidRootPart and (game.Players.LocalPlayer.Character.HumanoidRootPart.Position - v.HumanoidRootPart.Position).Magnitude < 5 then
        game.Players.LocalPlayer.Character.Push.PushTool:FireServer(v)
                end
    end)
    end
end
...
addcmd('massplay',{},function(args, speaker)
...
addcmd('timeposition',{},function(args, speaker)
local player = game.Players.LocalPlayer.Character
local localplayer = game.Players.LocalPlayer
...
addcmd('autoflingpirate',{},function()
local character = game.Players.LocalPlayer.Character
local HatName = "PirateCaptain_HatAccessory"
--Pony Dreads
--PirateCaptain_HatAccessory
--shownape hat drop
...
if v:IsA("Accessory") then
findfirstchildofclass(v.Handle,"Vector3Value"):Destroy()
end
end
findfirstchildofclass(game.Players.LocalPlayer.Character.Humanoid,"NumberValue"):Destroy()
wait(1)
end
wait(0.1)
for i,v in pairs(game.Players.LocalPlayer.Character:GetChildren()) do
if v:IsA("Accessory") then
findfirstchildofclass(v.Handle,"Attachment"):Destroy()
end
end
wait(0.1)
findfirstchildofclass(game.Players.LocalPlayer.Character.Humanoid,"NumberValue"):Destroy()
...
local obj = findfirstchildofclass(game.Players.LocalPlayer.Character,"Accessory")
for i=1,10 do
for i,v in pairs(game.Players:GetChildren()) do
pcall(function()
local ch = v.Character.HumanoidRootPart.CFrame
for i=1,5 do
obj.Velocity = Vector3.new(0,-10000,0)
obj.CFrame = ch
...
--coroutine.wrap(function()
    ...
    local bp = Instance.new("BodyPosition",HatName.Handle)
...
--bp.MaxForce = Vector3.new(math.huge,math.huge,math.huge)
    ...
    local bt = Instance.new("BodyThrust",HatName.Handle)
    HatName.Handle.AssemblyAngularVelocity = Vector3.new(9e9,9e9,9e9)
    bt.Force = Vector3.new(9e11,9e11,9e11)
    ...
    while true do
        ...
        HatName.Handle.CanCollide = false
       -- bp.Position = game.Players.LocalPlayer:GetMouse().Hit.p
    end
--end)()
...
HatName.Handle.Transparency = 0.9
local Outline = Instance.new("SelectionBox", HatName.Handle)

Outline.LineThickness = 0.05
game:GetService("RunService").Heartbeat:Connect(function()
local s = 5
local yes = tick() % s / s 
Outline.Color3 = Color3.fromHSV(yes, 2, 2)
end)

Outline.Adornee = HatName.Handle
_G.FlingPart = HatName.Handle
...
local s = 1
local yes = tick() % s / s 
Outline.Color3 = Color3.fromHSV(yes, 1, 1)
end)
...
addcmd('autoflingR6',{},function(args, speaker)
local character = game.Players.LocalPlayer.Character
local HatName = "Pony Dreads"
--Pony Dreads
--PirateCaptain_HatAccessory
--shownape hat drop
game.Players.LocalPlayer.Character = nil
...
pcall(function()
player["HatAccessory"].Handle.OriginalSize:Destroy()
player["RedcliffKnight_HelmetAccessory"].Handle.OriginalSize:Destroy()
player["PirateCaptain_HatAccessory"].Handle.OriginalSize:Destroy()
end)
...
addcmd('givehat',{},function(args, speaker)
	local players = getPlayer(args[1], speaker)
	for i,v in pairs(players) do
		givehat(speaker,Players[v])
	end
end)
